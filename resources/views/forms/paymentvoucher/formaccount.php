...
if( !empty($data) ){
    $formAction = '/totalpayments/'.$data->id;
    // if(!empty($item['logo'])){
...
// $idtype = !empty( $data->type_id )? $data->type_id:'1';
// $typies = array();
// $typies[] = array('id'=>1, 'name'=>'เงินโอน');
// $typies[] = array('id'=>2, 'name'=>'เงินสด');
// // $typies[] = array('id'=>3, 'name'=>'เช็ค');
//
// $type = '';
// foreach ($typies as $key => $value) {
//     $activies = $idtype==$value['id']? ' selected="1"':'';
//     $type .= '<option'.$activies.' value="'.$value['id'].'">'.$value['name'].'</option>';
// }
$banklist = '';
    ...
    <div class="col-12">
      ...
      <div class="row">
        <div class="controls col-12 mb-1">
          <textarea id="input_remark"  class="form-control" name="note" '.$conf.'>'.$note.'</textarea>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="alert alert-danger" role="alert">
            <div class="custom-control custom-radio text-dark" style="margin-left:-24px;">
              <input type="radio" id="status2" name="status" class="custom-control-input" value="3 " '.$status3.' '.$conf.'>
              <label class="custom-control-label font-weight-bold" for="status2">อนุมัติใบสำคัญจ่าย</label>
            </div>
            <div class="custom-control custom-radio text-dark" style="margin-left:-24px;">
              <input type="radio" id="status1" name="status" class="custom-control-input" value="2" '.$status2.' '.$conf.'>
              <label class="custom-control-label font-weight-bold" for="status1">ยกเลิกใบสำคัญจ่าย (กรุณากรอกหมายเหตุด้วย)</label>
            </div>
            <textarea id="input_remark"  class="form-control" name="remark" '.$conf.' style="margin-top:5px;" >'.$remark.'</textarea>
          </div>
        </div>
      </div>
