...
class ApiUserController extends Controller
{
    public function index(MDB $db, Request $request)
    {
        ...
        $res['items'] = $this->ui->item('UserDatatable')->init($res['data'], $res['options'] );
    ...
    public function reset_password(MDB $db, Request $request, $id)
    {
        ...
        $arr['code'] = 422;
        $arr['message'] = 'รหัสผ่านเดิมของคุณไม่ถูกต้อง กรุณาลองใหม่';
    ...
    public function changestatus(MDB $db,Request $request)
    {
      $data = MDB::findOrFail($request->id);
      if($request->status == 1){
        $data->status = 0;
        // $data->update();
      }else {
