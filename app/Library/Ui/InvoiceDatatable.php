...
class InvoiceDatatable extends Ui
{
    ...
    public function keys()
    {
        ...
        $key[] = array('id'=>'type', 'label'=>'ประเภทการโอน', 'cls'=>'td-name');
        $key[] = array('id'=>'total', 'label'=>'รวมเงิน', 'cls'=>'td-phone');
    ...
    public function status($data)
    {
        ...
        // else{
            switch ($data->status) {
                case 0:
                    $val = '<div class="ui-status danger" data-ref="status" data-type="status">ไม่อนุมัติ</div>';
                    break;
    
                case 1:
                    $val = '<div class="ui-status success" data-ref="status" data-type="status">อนุมัติ</div>';
                    break;
            
                case 2:
                    $val = '<div class="ui-status danger" data-ref="status" data-type="status">รอตรวจสอบ</div>';
                    break;
            }
        // }
    ...
    public function action($data)
    {
        ...
        if( $data->status==2 ){
            $btn .= '<a href="'.asset( "/invoice/{$id}" ).'"  class="btn btn-sm btn-warning ml-1" title="แก้ไข"><i class="fa fa-pencil"> ตรวจสอบข้อมูล</i></a>';
        }
         $btn .= '<a href="'.asset( "/invoice/pdfpercent/{$id}" ).'"  class="btn btn-sm btn-success ml-1" title="ใบกำกับภาษี"><i class="fa fa-file"> ใบกำกับภาษี</i></a>';
        return $btn;
    }
}
