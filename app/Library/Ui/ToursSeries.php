...
class ToursSeries extends Ui
{
    ...
    public function keys()
    {
    	...
    	$key[] = array('id'=>'wholesale_name', 'label'=>'โฮลเซลล์', 'cls'=>'td-content');
    ...
    public function _groupname($data)
    {
        ...
        if( !empty($data['gallery']) ){
            ...
            if( !empty($gallery[0]['path']) ){
                $picture = '<img src="'. asset("storage/{$gallery[0]['path']}").'" alt="" />';
            }
            elseif( !empty($gallery[0]['url']) ){
                $picture = '<img src="'. $gallery[0]['url'] .'" alt="" />';
            }
        }
        ...
        return '<div class="media">
            ...
            <div class="media-body">'.
                ...
                '<div class="mt-1"><a href="'.asset('/tours/series/'.$data['id']).'"><strong ref="name">'.$data['name'].'</strong></a></div>'.
                // target="_blank"
                '<div class="y-ellipsis clamp-2" style="font-size:13px;"><span ref="highlight">'. $this->fn->q('text')->more(strip_tags($data['highlight'])).'</span></div>'.
            '</div>
        </div>';
    }
    ...
    {
    	$val = '<a href="'.asset('/tours/series/'.$data['id']).'" target="_blank" class="btn btn-sm btn-primary" title="แก้ไข"><i class="fa fa-pencil"></i></a>';
		$val .= '<a href="'.asset('/tours/series/'.$data['id'].'/delete').'" data-plugin="lightbox" class="btn btn-sm btn-danger ml-2" title="ลบ"><i class="fa fa-remove"></i></a>';
